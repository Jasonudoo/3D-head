...
def main(_):
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        img, label = different.read_and_decode("../data1/tfrecord-3/")
        print('img=', img)
        ...
        summaries.add(tf.summary.scalar('accuracy', accuracy_add))
        #summaries.add(tf.summary.scalar('global_step', global_step))
        summaries.add(tf.summary.scalar('eval/Loss', loss_add))
       # summaries |= set(tf.get_collection(tf.GraphKeys.SUMMARIES))
        #summaries.add(tf.summary.image('image', batch_speech))
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for batch_num in range(num_batches_per_epoch):
                step += 1
                accuracy_add_average, val, l, loss_add_average, training_step, _, _, total_right_value, logits_value, trained_label_value, learning_rate_value, summary= sess.run(
                    [accuracy_add, batch_speech, batch_labels, loss_add, global_step, train_op, is_training,total_right, logits, trained_label, learning_rate, summary_op],
