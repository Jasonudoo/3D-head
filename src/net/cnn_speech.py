...
def speech_cnn(inputs, num_classes=1000,
    ...
    with tf.variable_scope(scope, 'net', [inputs]) as sc:
        ...
        with tf.contrib.framework.arg_scope([tf.contrib.layers.conv3d, tf.contrib.layers.max_pool2d],
            ...
            ############ Conv-1 ###############
            net = slim.conv3d(inputs, 16, [1, 3, 3], stride=[1, 1, 1], scope='conv11')
            print("net1:", inputs)
            net = PReLU(net, 'conv11_activation')
            net = slim.conv3d(net, 16, [1, 3, 3 ], stride=[1, 1, 1], scope='conv12')
            net = PReLU(net, 'conv12_activation')
            net = tf.nn.max_pool3d(net, strides=[1, 1, 1, 2, 1], ksize=[1, 1, 1, 2, 1], padding='VALID', name='pool1')
            print("net2:", net)
            ############ Conv-2 ###############
            net = slim.conv3d(net, 32, [3, 3, 3 ], stride=[1, 1, 1], scope='conv21')
            net = PReLU(net, 'conv21_activation')
            net = slim.conv3d(net, 32, [3, 3, 3 ], stride=[1, 1, 1], scope='conv22')
            net = PReLU(net, 'conv22_activation')
            net = tf.nn.max_pool3d(net, strides=[1, 1, 1, 2, 1], ksize=[1, 1, 1, 2, 1], padding='VALID', name='pool2')
            print("net3:", net)
            ############ Conv-3 ###############
            net = slim.conv3d(net, 64, [3, 3, 3 ], stride=[1, 1, 1], scope='conv31')
            net = PReLU(net, 'conv31_activation')
            net = slim.conv3d(net, 64, [1, 3, 3 ], stride=[1, 1, 1], scope='conv32')
            net = PReLU(net, 'conv32_activation')
            net = tf.nn.max_pool3d(net, strides=[1, 1, 2, 2, 1], ksize=[1, 1, 2, 2, 1], padding='VALID', name='pool3')
            print("net4:", net)
            ############ Conv-4 ###############
            net = slim.conv3d(net, 128, [3, 3, 3 ], stride=[1, 1, 1], scope='conv41')
            net = PReLU(net, 'conv41_activation')
            net = slim.conv3d(net, 128, [1, 3, 3 ], stride=[1, 1, 1], scope='conv42')
            net = PReLU(net, 'conv42_activation')
            net = tf.nn.max_pool3d(net, strides=[1, 1, 2, 2, 1], ksize=[1, 1, 2, 2, 1], padding='VALID', name='pool4')
            print("net5:", net)
            ############ Conv-5 ###############
            net = slim.conv3d(net, 128, [1, 208, 45], stride=[1, 1, 1], normalizer_fn=None, scope='conv51')
