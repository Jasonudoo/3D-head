...
def speech_cnn(inputs, num_classes=1000,
    ...
    with tf.variable_scope(scope, 'net', [inputs]) as sc:
        ...
        with tf.contrib.framework.arg_scope([tf.contrib.layers.conv2d, tf.contrib.layers.max_pool2d],  ####max_pool2d
            ...
            ############ Conv-2 ###############
            net = slim.conv3d(net, 128, [1, 3, 3], stride=[1, 1, 1], scope='conv21')
            ...
            net = slim.conv3d(net, 512, [1, 3, 3], stride=[1, 1, 1], scope='conv42')
            net = PReLU(net, 'conv42_activation')
            print("4.2:", net)

            net = slim.conv3d(net, 512, [1, 3, 3], stride=[1, 1, 1], scope='conv43')
            ...
            ############ Conv-5 ###############
            net = slim.conv3d(net, 512, [1, 3, 3], stride=[1, 1, 1], scope='conv51')
